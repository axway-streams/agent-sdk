...
func Test_SubscriptionPropertyBuilder_Build_with_valid_values(t *testing.T) {
	tests := []struct {
		name        string
		builder     SubscriptionPropertyBuilder
		...
		{"Full Array property",
			NewSubscriptionSchemaPropertyBuilder().
				...
				SetAPICRefField("APICRefField").
				IsArray().
				AddArrayItem(NewSubscriptionSchemaPropertyBuilder().
					SetName("ItemName").
					IsString()).
				SetMinArrayItems(0).
				SetMaxArrayItems(1),
